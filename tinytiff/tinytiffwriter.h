...
extern "C" {
    ...
    */
    extern const char* TinyTIFFWriter_getLastError(TinyTIFFWriterFile* tiff);
    ...
    */
    extern int TinyTIFFWriter_wasError(TinyTIFFWriterFile* tiff);
    ...
    /*! \brief returns TINYTIFF_TRUE (non-zero) when there was no error in the last function call, or TINYTIFF_FALSE if there was an error
        ...
        */
    extern int TinyTIFFWriter_success(TinyTIFFWriterFile* tiff);
    ...
    /*! \brief create a new TIFF file
      ...
      */
    extern TinyTIFFWriterFile* TinyTIFFWriter_open(const char* filename, uint16_t bitsPerSample, enum TinyTIFFWriterSampleFormat sampleFormat, uint16_t samples, uint32_t width, uint32_t height, enum TinyTIFFWriterSampleInterpretation sampleInterpretation);
     ...
     */
    extern int TinyTIFFWriter_writeImageMultiSample(TinyTIFFWriterFile* tiff, const void* data, enum TinyTIFFSampleLayout inputOrganisation, enum TinyTIFFSampleLayout outputOrganization);
    ...
    /*! \brief write a new image to the give TIFF file, in planar configuration, i.e. the image data is reorganized from RGBRGBRGB to RRR...GGG...BBB... before writing.
       ...
       \note Note that the reorganization from chunky to planar requires additional
             ...
             \image html tinytiffwriter_performance_rgb.png
    */
    extern int TinyTIFFWriter_writeImagePlanarReorder(TinyTIFFWriterFile* tiff, const void* data);
    ...
    /*! \brief write a new image to the give TIFF file, in planar configuration, i.e. the image data is reorganized from RGBRGBRGB to RRR...GGG...BBB... before writing.
        ...
        \note Note that the reorganization from planar to chunky requires additional
              ...
              \image html tinytiffwriter_performance_rgb.png
    */
    extern int TinyTIFFWriter_writeImageChunkyReorder(TinyTIFFWriterFile* tiff, const void* data);
    ...
    /*! \brief Write a new image to the give TIFF file, in chunky configuration, expects the data to be chunky too.
        ...
        \return TINYTIFF_TRUE on success and TINYTIFF_FALSE on failure.
                An error description can be obtained by calling TinyTIFFWriter_getLastError().
    */
    extern int TinyTIFFWriter_writeImage(TinyTIFFWriterFile* tiff, const void* data);
    ...
    */
    extern void TinyTIFFWriter_close_withmetadatadescription(TinyTIFFWriterFile* tiff, double pixel_width, double pixel_height, double frametime, double deltaz);
    ...
    /*! \brief close a given TIFF file
        ...
        This function also releases memory allocated in TinyTIFFWriter_open() in \a tiff.
    */
    extern void TinyTIFFWriter_close(TinyTIFFWriterFile* tiff);
    ...
    /*! \brief close a given TIFF file and write the given string into the IMageDescription tag of the first frame in the file.
        ...
        This function also releases memory allocated in TinyTIFFWriter_open() in \a tiff.
     */
    extern void TinyTIFFWriter_close_withdescription(TinyTIFFWriterFile* tiff, const char* imageDescription);
#ifdef __cplusplus
}
#endif
